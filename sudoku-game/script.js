...
const UI = {
    ...
    updatePersonalBests() {
        ...
        Config.getDifficultyNames().forEach(diff => {
            ...
            if (el) el.textContent = time ? State.getFormattedTime.call({timer: time}) : '-';
        });
    },
    ...
    renderLeaderboard(difficulty) {
        ...
        const leaderboard = Storage.getLeaderboard(difficulty);
    ...
    // 显示排行榜
    showLeaderboard() {
        this.renderLeaderboard(State.currentDifficulty);
...
const Game = {
    ...
    // 记录成绩
    recordScore() {
        Storage.addScore(State.currentDifficulty, State.currentPlayer, State.victoryTime);
        Storage.updatePersonalBest(State.currentDifficulty, State.victoryTime);
        UI.hideVictory();
        UI.showLeaderboard();
        UI.updatePersonalBests();
    },
...
function applyTheme(theme) { Game.applyTheme(theme); }
...
function switchTab(diff) { Game.switchTab(diff); }
function recordScore() { Game.recordScore(); }
